...
component_title: "GCP BigQuery"
description: "The Vector `gcp_bigquery` sink batches `log` events to Google Cloud Platform's BigQuery service via the REST Interface."
event_types: ["log"]
function_category: "transmit"
issues_url: https://github.com/timberio/vector/issues?q=is%3Aopen+is%3Aissue+label%3A%22sink%3A+gcp_bigquery%22
min_version: null
operating_systems: ["Linux","MacOS","Windows"]
service_name: "GCP BigQuery"
sidebar_label: "gcp_bigquery|[\"log\"]"
source_url: https://github.com/timberio/vector/blob/master/src/sinks/gcp/bigquery.rs
status: "beta"
title: "GCP BigQuery Sink"
unsupported_operating_systems: []
---

The Vector `gcp_bigquery` sink [batches](#buffers--batches) [`log`][docs.data-model.log] events to [Google Cloud Platform's BigQuery service][urls.gcp_bigquery] via the [REST Interface][urls.gcp_bigquery_rest].
...
<!--
     ...
     website/docs/reference/sinks/gcp_bigquery.md.erb
-->
...
The filename for a Google Cloud service account credentials JSON file used to authenticate access to the bigquery project, dataset, and table. If this is unset, Vector checks the `$GOOGLE_APPLICATION_CREDENTIALS` environment variable for a filename. See [GCP Authentication](#gcp-authentication) for more info.
...
The dataset within the project to which to publish logs.
...
The project name to which to publish log records.
...
The table within the project to which to publish log records.
...
The filename for a Google Cloud service account credentials JSON file used to authenticate access to the bigquery project, dataset, and table. See [GCP Authentication](#gcp-authentication) for more info.
...
The `gcp_bigquery` sink buffers & batches data as
...
with the Vector team by [opening an issie][urls.new_gcp_bigquery_sink_issue].
